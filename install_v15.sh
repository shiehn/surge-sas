...
echo "===================================="
echo "Surge XT v1.5.0 Installation Script"
echo "===================================="
echo ""

# Check if running as root
if [ "$EUID" -ne 0 ]; then 
    echo "Please run with sudo: sudo ./install_v15.sh"
    exit 1
fi

echo "Step 1: Verifying v1.5.0 build exists..."
if [ ! -f "/Users/stevehiehn/surge-sas/build-arm64/surge_xt_products/Surge XT.vst3/Contents/MacOS/Surge XT" ]; then
    echo "ERROR: v1.5.0 build not found!"
    exit 1
fi

if strings "/Users/stevehiehn/surge-sas/build-arm64/surge_xt_products/Surge XT.vst3/Contents/MacOS/Surge XT" | grep -q "S&S Fork v1.5.0"; then
    echo "✓ Found v1.5.0 build"
else
    echo "ERROR: Build is not v1.5.0!"
    exit 1
fi
...
if strings "/Library/Audio/Plug-Ins/VST3/Surge XT.vst3/Contents/MacOS/Surge XT" | grep -q "S&S Fork v1.5.0"; then
    echo "✅ SUCCESS! v1.5.0 is now installed"
    echo ""
...
echo ""
echo "The plugin will show: S&S Fork v1.5.0 (Router Client)"
echo ""
echo "Key Changes in v1.5.0:"
echo "- Memory-based instance IDs for stable multi-instance routing"
echo "- Each plugin instance gets a unique ID: surge-<memory_address>"
echo ""